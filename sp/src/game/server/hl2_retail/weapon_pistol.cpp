...
class CWeaponPistolRetail : public CBaseHLCombatWeapon
{
	DECLARE_DATADESC();

public:
	DECLARE_CLASS( CWeaponPistolRetail, CBaseHLCombatWeapon );

	CWeaponPistolRetail(void);
...
IMPLEMENT_SERVERCLASS_ST(CWeaponPistolRetail, DT_WeaponPistolRetail)
END_SEND_TABLE()

LINK_ENTITY_TO_CLASS( weapon_pistol, CWeaponPistolRetail );
PRECACHE_WEAPON_REGISTER( weapon_pistol );

BEGIN_DATADESC( CWeaponPistolRetail )
...
acttable_t	CWeaponPistolRetail::m_acttable[] = 
{
...
IMPLEMENT_ACTTABLE( CWeaponPistolRetail );

#ifdef MAPBASE
// Allows Weapon_BackupActivity() to access the pistol's activity table.
acttable_t *GetPistolActtable()
{
	return CWeaponPistolRetail::m_acttable;
}

int GetPistolActtableCount()
{
	return ARRAYSIZE(CWeaponPistolRetail::m_acttable);
}
#endif
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CWeaponPistolRetail::CWeaponPistolRetail( void )
{
...
// Purpose:
//-----------------------------------------------------------------------------
bool CWeaponPistolRetail::Deploy(void)
{
	if (weapons_2003leakbehaviour.GetInt() == 1)
	{ 
		// temp code
		engine->ServerCommand("use weapon_crowbar\n");
		engine->ServerCommand("give weapon_pistol_old\n");
		engine->ServerCommand("use weapon_pistol_old\n");
		UTIL_Remove(this);
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPistolRetail::Precache( void )
{
	BaseClass::Precache();
}
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponPistolRetail::Operator_HandleAnimEvent( animevent_t *pEvent, CBaseCombatCharacter *pOperator )
{
	switch( pEvent->event )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPistolRetail::FireNPCPrimaryAttack( CBaseCombatCharacter *pOperator, Vector &vecShootOrigin, Vector &vecShootDir )
{
	CSoundEnt::InsertSound( SOUND_COMBAT|SOUND_CONTEXT_GUNFIRE, pOperator->GetAbsOrigin(), SOUNDENT_VOLUME_PISTOL, 0.2, pOperator, SOUNDENT_CHANNEL_WEAPON, pOperator->GetEnemy() );
...
//-----------------------------------------------------------------------------
void CWeaponPistolRetail::Operator_ForceNPCFire( CBaseCombatCharacter *pOperator, bool bSecondary )
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponPistolRetail::DryFire( void )
{
	WeaponSound( EMPTY );
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponPistolRetail::PrimaryAttack( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPistolRetail::UpdatePenaltyTime( void )
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPistolRetail::ItemPreFrame( void )
{
	UpdatePenaltyTime();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPistolRetail::ItemBusyFrame( void )
{
	UpdatePenaltyTime();
...
//-----------------------------------------------------------------------------
void CWeaponPistolRetail::ItemPostFrame( void )
{
...
// Output : int
//-----------------------------------------------------------------------------
Activity CWeaponPistolRetail::GetPrimaryAttackActivity( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CWeaponPistolRetail::Reload( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPistolRetail::AddViewKick( void )
{
